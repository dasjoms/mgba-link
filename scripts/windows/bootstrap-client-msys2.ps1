...
Invoke-Step -Description "Prepare runtime bundle in '$runtimeDir'" -Action {
    ...
    $bundleScriptLines = @(
        ...
        '  deploy_args=(--release --no-translations --no-compiler-runtime)'
        '  if [[ ! -f "/mingw64/bin/libGLESv2.dll" || ! -f "/mingw64/bin/libEGL.dll" ]]; then'
        '    echo "note: ANGLE runtime DLLs are not available in /mingw64/bin; using --no-angle for windeployqt" >&2'
        '    deploy_args+=(--no-angle)'
        '  fi'
        '  "$deploy_tool" "${deploy_args[@]}" "./$exe"'
        'else'
